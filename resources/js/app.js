...
function buildChannelRow(channel, programs) {
    // Use lane-0 programs only for single-row display
    ...
    for (const p of lane0) {
        ...
        const isNow   = Date.now() >= new Date(p.start_time) && Date.now() < new Date(p.end_time);
        ...
        <div onclick="openDetail(${p.id})"
             style="position:absolute;top:5px;left:${left}px;width:${width}px;height:${ROW_H-10}px;
                    ...
                    background:${isNow ? 'rgba(79,70,229,0.22)' : 'rgba(30,30,46,0.6)'};
                    border:1px solid ${isNow ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.06)'};
                    transition:border-color .15s,background .15s;padding:5px 7px;display:flex;flex-direction:column;justify-content:space-between;"
             onmouseenter="this.style.borderColor='rgba(99,102,241,0.4)';this.style.background='rgba(40,40,60,0.9)'"
             onmouseleave="this.style.borderColor='${isNow ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.06)'}'
                          ;this.style.background='${isNow ? 'rgba(79,70,229,0.22)' : 'rgba(30,30,46,0.6)'}'">
            <div style="font-size:10px;font-weight:600;color:#cbd5e1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${p.title}</div>
            <div style="display:flex;align-items:center;justify-content:space-between;">
                <span style="font-size:9px;color:#475569;">${fmtNPT(p.start_time)}</span>
                ${hasRem ? '<i class="fa-solid fa-bell" style="font-size:8px;color:#818cf8;"></i>' : ''}
            </div>
        </div>`;
    }
    return html;
...
function renderEPG() {
    ...
    for (const ch of state.channels) {
        ...
        channelLabelsHtml += `
        <div style="height:${ROW_H}px;display:flex;align-items:center;justify-content:space-between;padding:0 10px;border-bottom:1px solid rgba(255,255,255,0.04);gap:6px;flex-shrink:0;">
            <div style="display:flex;align-items:center;gap:6px;min-width:0;">
                <span style="width:8px;height:8px;border-radius:50%;flex-shrink:0;background:${ch.logo_color}"></span>
        ...
        rowsHtml += `
        <div style="position:relative;height:${ROW_H}px;width:${GRID_W}px;border-bottom:1px solid rgba(255,255,255,0.04);">
            ${buildVerticalGridLines()}
            ${buildChannelRow(ch, programs)}
        </div>`;
    }


    return `
    <div style="flex:1;display:flex;flex-direction:column;min-height:0;background:#0a0a0f;">
        <!-- Toolbar -->
    ...
    <div style="flex:1;overflow-y:auto;padding:24px;background:#06060a;display:flex;flex-direction:column;align-items:center;">
        <div style="width:100%;max-width:960px;">
            ...
            <div style="position:relative;width:100%;padding-top:56.25%;background:#000;border-radius:16px;overflow:hidden;border:1px solid #1e1e2e;box-shadow:0 25px 60px rgba(0,0,0,0.8);">
                <video controls autoplay loop muted style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;">
...
// ─── Actions ─────────────────────────────────────
window.changeDate = function(d) { state.currentDate = d; loadProgramsForDate(d).then(renderApp); };
window.shiftDate  = function(dir) {
    ...
    if (i === -1) return;
    const ni  = i + dir;
    if (ni >= 0 && ni < all.length) { state.currentDate = all[ni]; loadProgramsForDate(all[ni]).then(renderApp); }
};
